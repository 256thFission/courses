...
```{r}
export_instructor_results(
  results = instructor_results, 
  raw_data = all_data,
  file_path = "instructor_table.csv"
)
```

```{r}
head(read.csv("instructor_table.csv"))
```

```{r}
# For instructor_results data in memory
na_counts <- sapply(instructor_results$data, function(x) sum(is.na(x)))
na_counts_df <- data.frame(
  Column = names(na_counts),
  NA_Count = unname(na_counts)
)
# Sort by number of NAs (descending)
na_counts_df <- na_counts_df[order(-na_counts_df$NA_Count), ]
print(na_counts_df)

# To focus on specific columns like Q6 related ones:
q6_cols <- grep("Q6|percent_top_n", names(instructor_results$data), value = TRUE)
q6_na_counts <- sapply(instructor_results$data[q6_cols], function(x) sum(is.na(x)))
print(data.frame(Column = names(q6_na_counts), NA_Count = unname(q6_na_counts)))
```
